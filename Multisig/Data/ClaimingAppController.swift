...
class ClaimingAppController {
    ...
    func combine(transactions: [Transaction], safe: Safe) -> Transaction? {
        ...
        if safe.contractVersion == nil || isSafe1_3_0 {
            ...
            data = MultiSendCallOnly.multiSend(transactions: packedTransactions).encode()
            to = try! Address(SafeDeployments.Safe.Deployment.find(contract: .MultiSendCallOnly, version: .v1_1_1)!.address(for: configuration.chainId)!)
        }
