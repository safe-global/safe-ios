...
public class HDNode {
    public struct HDversion {
        public var privatePrefix: Data = Data(hex: "0x0488ADE4")
        public var publicPrefix: Data = Data(hex: "0x0488B21E")
        public init() {}
    }
    ...
    public static func privateToPublic(_ privateKey: Data, compressed: Bool = false) -> Data? {
        guard let publicKey = SECP256K1.privateToPublic(privateKey:  privateKey,
                                                        compressed: compressed) else { return nil }
        return publicKey
    }
...
extension HDNode {
    ...
    public func derive (path: String, derivePrivateKey: Bool = true) -> HDNode? {
        ...
        for component in components[firstComponent ..< components.count] {
            ...
            guard let newNode = currentNode.derive(index: index,
                                                   derivePrivateKey: derivePrivateKey,
                                                   hardened: hardened) else { return nil }
            currentNode = newNode
        }
        return currentNode
    }
